...
def scheduleProcs(tslice, cpuQ):
    ...
    while (cpuQ.empty() != True):
        ...
        PID,exectime,total = proc.split(",")
        
        # Convert exectime to an integer
        exectime = int(exectime)
        
        print("Getting next process - Process ", PID," has ", exectime," instructions out of",total,"to execute ")
        
        # Initialize the timer
        timer = 0
        t = int(exectime)/int(total)
        if t > 0.5:
            actual = tslice * 2 
        else:
            actual = tslice
        ...
        while (timer < actual) and (exectime > 0):
        ...
        if (exectime > 0):
            ...
            proc = PID + "," + str(exectime)+ "," + str(total)
            
            # Put the process back in the queue
            cpuQ.put(proc)
            
            print("Put process ", PID," back in queue with ", exectime," instructions out of ",total,"left to execute")
        else:
